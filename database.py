...
def connect_database():
    ...
    global conn
    try:
        conn=pymysql.connect(host='localhost',user='root',password='Pranjal@2005')
        mycursor=conn.cursor()
    except:
        messagebox.showerror("Error","Something went wrong")
        return
    mycursor.execute('CREATE DATABASE IF NOT EXISTS employee_data')#Creating database
    mycursor.execute('USE employee_data')#use the created database
    ...
    #making table with the database
